...
```{r}
col.vec <- rainbow(out1$summary.stats$nSpecies)
```
...
```{r}
sim1 <- confetti.run(pars = parsets[1,], nTrees = 2500, Xext = 250, Yext = 250, nRep = 5, avg = T)
sim2 <- confetti.run(pars = parsets[2,], nTrees = 2500, Xext = 250, Yext = 250, nRep = 5, avg = T)
sim3 <- confetti.run(pars = parsets[3,], nTrees = 2500, Xext = 250, Yext = 250, nRep = 5, avg = T)
sim4 <- confetti.run(pars = parsets[4,], nTrees = 2500, Xext = 250, Yext = 250, nRep = 5, avg = T)
sim5 <- confetti.run(pars = parsets[5,], nTrees = 2500, Xext = 250, Yext = 250, nRep = 5, avg = T)
```
...
Fr.dat <- data.frame(Fr = as.numeric(Fr),
                     r = 1:100,
                     disp = rep(parsets2$disp, each = Fr.length)
)
...
SAR.dat <- data.frame(nSpec = as.numeric(SAR),
                      Area_ha = sim.list2[[1]]$Area / 1e4,
                      disp = rep(parsets2$disp, each = SAR.length)
)
...
```{r}
sim1 <- confetti.run(pars = parsets3[1,], nTrees = 500, Xext = 250, Yext = 250, nRep = 1, avg = F)
sim2 <- confetti.run(pars = parsets3[2,], nTrees = 500, Xext = 250, Yext = 250, nRep = 1, avg = F)
```
